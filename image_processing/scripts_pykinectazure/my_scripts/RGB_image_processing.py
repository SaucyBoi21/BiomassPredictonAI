...
path = f'C:/Users/jcard/OneDrive - University of Georgia/kinect_imaging/scripts_pykinectazure/my_scripts/rgb_imgs./{img_name}.jpg'
...
img, path, img_filename = pcv.readimage(filename=path, mode="native")
pcv.plot_image(img)

# If we need to rotate the image, use this code
...
crop = pcv.crop(img=img, x=120, y=50, h=750, w=1600)
pcv.plot_image(crop)
#pcv.print_image(crop,'C:/Users/jcard/OneDrive - University of Georgia/side_projects/seedlings_Ferrarezi_Lab/process_imgs/crop.jpg')
...
colorspace_img = pcv.visualize.colorspaces(rgb_img=crop,original_img=False)
...
hsv_s = pcv.rgb2gray_hsv(rgb_img=crop, channel='s')
...
a_fill = pcv.fill(bin_img=thresh, size=100)
pcv.plot_image(a_fill)
...
obj, obj_hierarchy = pcv.find_objects(img=crop, mask=a_fill)


# ROIS will create a list with all the 98 position in the seeding tray. 
# Use len(rois) for the expected total seedling count. 
rois, roi_hierarchy = pcv.roi.multi(img=crop, coord=(298,268), radius=85, 
                                    spacing=(258, 255), nrows=7, ncols=14)
