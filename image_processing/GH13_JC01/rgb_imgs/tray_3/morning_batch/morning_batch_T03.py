...
# Paths where final imgs of the results and data base for each tray will be stored respectively
#img_results = 'C:/Users/jcard/OneDrive - University of Georgia/side_projects_ideas/seedlings_Ferrarezi_Lab/results_img'
#csv_results = 'C:/Users/jcard/OneDrive - University of Georgia/side_projects_ideas/seedlings_Ferrarezi_Lab/results_csv'

filenames = sorted(glob.glob('C:/Users/jcard/OneDrive - University of Georgia/kinect_imaging/GH13_JC01/rgb_imgs/tray_3/morning_batch/raw_images/T03*.jpg'))
...
#img_name to avoid changing name in every path or when storing results  
    ...
    # Divides img_name after each underscore to extract each metadata 
    metadata = img_name.rsplit('_',3)
    
    # give img location use "/" instead of "\"
    path_analyzed_imgs = 'C:/Users/jcard/OneDrive - University of Georgia/kinect_imaging/GH13_JC01/rgb_imgs/tray_3/morning_batch/analyzed_imgs'
    path_result = 'C:/Users/jcard/OneDrive - University of Georgia/kinect_imaging/GH13_JC01/rgb_imgs/tray_3/morning_batch/results_csv'
    path_segmentation = 'C:/Users/jcard/OneDrive - University of Georgia/kinect_imaging/GH13_JC01/rgb_imgs/tray_3/morning_batch/morning_segmentation.txt'
    
    # tray_number = 1
    # pcv.params.debug = "print"
    # Let's develop the code for a single image first
    # PCV is reading this img as BGR instead of RGB
    raw_img, path, img_filename = pcv.readimage(filename= filename, mode="native")
    pcv.plot_image(raw_img)
    
    # (X and Y are starting X,Y coordinate respectively)
    # h = y_axis total lenght , w = x_axis total lenght
    img = pcv.crop(img=raw_img, x=0, y=110, h=900, w=1920)
    pcv.plot_image(img)
    ...
    #hsv_s = pcv.rgb2gray_hsv(rgb_img=img, channel='h')
    ...
    naive = pcv.naive_bayes_classifier(img,path_segmentation)
    ...
    # thresh = pcv.threshold.binary(gray_img=hsv_s, threshold=90, max_value=255, object_type='dark')
    ...
    h_fill = pcv.fill(bin_img=naive['plant'], size=100)
    ...
    plant_observation = list(plant_data.keys())
     ...
     ## Germination rates are not necessary in this analysis (if required:GERMINATION RATE HERE)
    ...
    data_set.insert(1, 'img_id', metadata[0])
    data_set.insert(2, 'date', metadata[1])
    data_set.insert(3, 'time', metadata[2])    
    ...
    data_set.to_csv(os.path.join(path_result,f'{img_name}_data_test.csv'))
    ...
    pcv.outputs.save_results(filename = f"C:/Users/jcard/OneDrive - University of Georgia/kinect_imaging/GH13_JC01/rgb_imgs/tray_3/morning_batch/csv_plantcv/pcv_{img_name}.csv", outformat="csv")
    pcv.outputs.clear()
